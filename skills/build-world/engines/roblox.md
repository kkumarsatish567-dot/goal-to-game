...
Roblox-specific instructions for Build World. The shared Thrixel asset pipeline remains in
...
Read [`roblox/MCP_AUTONOMY.md`](roblox/MCP_AUTONOMY.md) in full before starting. Its Studio-MCP
contract is authoritative for import, inspection, Play mode, console capture, input, screenshots,
and the no-human-in-the-loop completion gate.
...
6. Roblox Studio MCP is mandatory on the normal build path. Confirm a real attached Studio with
   `get_studio_state` (or the current equivalent) and a harmless `execute_luau` probe before
   spending Thrixel Cubes. A process handshake alone is not enough.
7. Drive import, hierarchy inspection, Play/Stop, console reads, gameplay input, and screenshots
   through Studio MCP. Human clicks may help debug a broken environment, but they do not satisfy
   the autonomous completion gate.
8. Publishing, API-key creation, paid uploads, and account permission changes remain explicit user
...
### Studio Importer: MCP-driven fallback path

The preferred autonomous ingest path is defined in `roblox/MCP_AUTONOMY.md`. Use Studio's 3D Importer only as an MCP-driven fallback when the Open Cloud + `InsertService:LoadAsset` path is blocked by a documented Roblox limitation. A person clicking Import is not the normal path and does not satisfy the completion gate.

When the MCP-driven importer fallback is used, verify exactly how the hierarchy, pivots and materials resolved.
...
### Open Cloud: preferred autonomous upload path
...
A Roblox Build World result is done only when a fresh checkout plus the documented toolchain lets
an agent execute the normal path end-to-end without a person operating Studio: connect to an active
Studio MCP instance, import and enumerate the real hierarchy, build/sync through Rojo, enter Play
mode, read console output, drive gameplay (including an independently moving imported part), capture
and inspect visual evidence, and rerun after fixes. Every Thrixel mesh must also pass preflight and
remain traceable through the manifest. Desktop/mobile performance claims require measured evidence.
