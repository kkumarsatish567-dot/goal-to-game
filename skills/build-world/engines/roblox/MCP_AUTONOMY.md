...
A minimal inspection shape for `execute_luau` is:

```luau
local InsertService = game:GetService("InsertService")
local HttpService = game:GetService("HttpService")

local MODEL_ASSET_ID = 0 -- replace from the completed Open Cloud operation
assert(MODEL_ASSET_ID > 0, "model asset id is required")

local container = InsertService:LoadAsset(MODEL_ASSET_ID)
container.Name = "ThrixelImportStaging"
...
for _, inst in container:GetDescendants() do
    if inst:IsA("MeshPart") then
        table.insert(meshes, {
            ...
            pivot = {inst:GetPivot():GetComponents()},
        })
    end
end

print("THRIXEL_IMPORT_MANIFEST=" .. HttpService:JSONEncode(meshes))
return #meshes
```
